...
func TestStackSendBufferSizeOption(t *testing.T) {
	...
	testCases := []struct {
		...
		err  func(tcpip.Error) tcpip.Error
	}{
		// Invalid configurations.
		{"min_below_zero", tcpip.SendBufferSizeOption{Min: -1, Default: sMin, Max: sMin}, func(err tcpip.Error) tcpip.Error {
			if _, ok := err.(*tcpip.ErrInvalidOptionValue); ok {
				return nil
			}
			return &tcpip.ErrInvalidOptionValue{}
		}},
		{"min_zero", tcpip.SendBufferSizeOption{Min: 0, Default: sMin, Max: sMin}, func(err tcpip.Error) tcpip.Error {
			if _, ok := err.(*tcpip.ErrInvalidOptionValue); ok {
				return nil
			}
			return &tcpip.ErrInvalidOptionValue{}
		}},
		{"default_below_min", tcpip.SendBufferSizeOption{Min: 0, Default: sMin - 1, Max: sMin - 1}, func(err tcpip.Error) tcpip.Error {
			if _, ok := err.(*tcpip.ErrInvalidOptionValue); ok {
				return nil
			}
			return &tcpip.ErrInvalidOptionValue{}
		}},
		{"default_above_max", tcpip.SendBufferSizeOption{Min: 0, Default: sMin + 1, Max: sMin}, func(err tcpip.Error) tcpip.Error {
			if _, ok := err.(*tcpip.ErrInvalidOptionValue); ok {
				return nil
			}
			return &tcpip.ErrInvalidOptionValue{}
		}},
		{"max_below_min", tcpip.SendBufferSizeOption{Min: sMin, Default: sMin + 1, Max: sMin - 1}, func(err tcpip.Error) tcpip.Error {
			if _, ok := err.(*tcpip.ErrInvalidOptionValue); ok {
				return nil
			}
			return &tcpip.ErrInvalidOptionValue{}
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if fn := tc.err; fn != nil {
				if want := fn(err); want != nil {
					t.Fatalf("s.SetOption(%+v) = %v, want: %s", tc.ss, err, want)
				}
			} else if err != nil {
				t.Fatalf("s.SetOption(%+v) = %s, want: nil", tc.ss, err)
			} else {
...
func TestFindRouteWithForwarding(t *testing.T) {
	...
	tests := []struct {
		...
		findRouteErr          func(tcpip.Error) tcpip.Error
	...
	}{
		{
			...
			findRouteErr: func(err tcpip.Error) tcpip.Error {
				if _, ok := err.(*tcpip.ErrNoRoute); ok {
					return nil
				}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if fn := test.findRouteErr; fn != nil {
				if want := fn(err); want != nil {
					t.Fatalf("FindRoute(%d, %s, %s, %d, false) = %s, want = %s", test.addrNIC, test.localAddr, test.netCfg.remoteAddr, test.netCfg.proto, err, want)
				}
			} else if err != nil {
				t.Fatalf("FindRoute(%d, %s, %s, %d, false) = %s, want = nil", test.addrNIC, test.localAddr, test.netCfg.remoteAddr, test.netCfg.proto, err)
			}
